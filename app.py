...
# ===================== Page Config & Theme =====================
st.set_page_config(
    page_title="AURA - Stress Level Predictor",
    page_icon="🧠",
    layout="wide",
...
with col3:
    if st.button("🌓", key="theme_toggle", help="Toggle Light/Dark mode"):
...
def load_artifacts():
    try:
        ...
        model = joblib.load(base_path / "model_rf.joblib")
        with open(base_path / "feature_order.json") as f:
...
# ===================== Show Landing Page =====================
st.markdown("""
    <div style='text-align: center; padding: 2rem; margin: 2rem 0; background-color: #f0f2f6; border-radius: 10px;'>
        <h1 style='font-size: 2.5rem;'>🧠 AURA - Stress Level Predictor</h1>
        <p style='font-size: 1.2rem;'>Your AI-powered stress management companion</p>
    </div>
""", unsafe_allow_html=True)
...
with col1:
    st.markdown("""
    ### 🎯 How it works
    1. Input your psychological factors
    2. Get instant stress predictions
...
def main():
    ...
    # ===================== Helper Functions =====================
    def apply_optimized_values(new_values):
        """Helper function to store optimized values that will be applied on next rerun"""
        st.session_state['optimized_values'] = new_values
        st.rerun()

    # At the start of the file, after default_values initialization, add:
    if 'optimized_values' in st.session_state:
        for k, v in st.session_state['optimized_values'].items():
            st.session_state[k] = v
        del st.session_state['optimized_values']
    ...
    # Run optimiser
    if st.button("✨ Optimise my stress", use_container_width=True, key="optimize_button"):  # Add unique key here too
        ...
        if len(changes) > 0:
            ...
            if st.button("Apply optimized values", use_container_width=True, key="apply_optimized"):
                optimized_values = {feat: int(new_row.iloc[0][feat]) for feat in FEATURE_ORDER}
                apply_optimized_values(optimized_values)
            st.success("✅ Click 'Apply optimized values' to update the sliders.")
        else:
